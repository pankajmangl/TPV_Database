CREATE OR REPLACE TRIGGER &TRANSFORMPV_USER..TRG_E2B_CFG_TRASFORM_PROFILES
/*****************************************************************************************
Company             : Vitrana
Author              : Pavan Kumar
Version Number      : 1.0
...
ON &TRANSFORMPV_USER..E2B_CFG_TRASFORM_PROFILES 
...
BEGIN
   ...
   BEGIN
      SELECT PAGE_ID
       ...
       WHERE TABLE_NAME = 'E2B_CFG_TRASFORM_PROFILES';
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         LN_PAGE_ID    := NULL;
   ...
   UPDATE E2B_CFG_TRASFORM_PROFILES_HIST
      SET EFFECTIVE_END_DATE = LD_SYSDATE,
          CURRENT_FLG = 0
    WHERE CURRENT_FLG = 1 AND PROFILE_ID = :NEW.PROFILE_ID;

   INSERT INTO E2B_CFG_TRASFORM_PROFILES_HIST
               (PROFILE_ID, PROFILE_NAME, DELETED,
                DELETED_DATE, CREATED_BY, CREATED_DATE,
                MODIFIED_BY, MODIFIED_DATE, IS_ACK,
                IS_BASE_PROFILE, DUP_CHECK_NAME, VERSION_NUMBER,
                AUDIT_ID, OPERATION_TYPE, EFFECTIVE_START_DATE,
                EFFECTIVE_END_DATE, CURRENT_FLG, INSERT_DATE
               )
        VALUES (:NEW.PROFILE_ID, :NEW.PROFILE_NAME, :NEW.DELETED,
                :NEW.DELETED_DATE, :NEW.CREATED_BY, :NEW.CREATED_DATE,
                :NEW.MODIFIED_BY, :NEW.MODIFIED_DATE, :NEW.IS_ACK,
                :NEW.IS_BASE_PROFILE, :NEW.DUP_CHECK_NAME, LV_VERSION_NUMBER,
