...
/******************************************************************************************************
  ...
  PROCEDURE P_GET_E2B_TRANSFORM_STATUS (
         ...
         := (CASE
                ...
                ELSE 'FROM E2B_TRANSFORM_STATUS E,
                           (SELECT SR.REPORT_ID, MSG.MESSAGEHEADER.MESSAGENUMB MESSAGENUMB, SR.SAFETYREPORTID,
                                   SR.MSG_ID, CM.CASE_NUM, SR.RECEIVER_AGENCY,SR.STATUS
   ...
   BEGIN
      V_SQL_SELECT    :=
         'SELECT E.MESSAGE_NUMBER, E.SAFETY_REPORT_ID, CASE WHEN E.REPORT_TYPE = ''Upgrade'' THEN
            NVL (CM.CASE_NUM, E.SAFETY_REPORT_ID)
            ELSE '||
             CASE WHEN PI_ACCESS_TYPE IN ('I', 'G') THEN
             'DECODE( SR.STATUS,102,NVL (CM.CASE_NUM, E.SAFETY_REPORT_ID),E.SAFETY_REPORT_ID) END CASE_NUM,'
             ELSE
             'DECODE( CM.STATUS,102,NVL (CM.CASE_NUM, E.SAFETY_REPORT_ID),E.SAFETY_REPORT_ID) END CASE_NUM,'
             END||' E.FOLLOWUP,
                   ...
                   E.E2B_FILE_NAME , case when UPPER(substr(e2b_html_name,instr(e2b_html_name,''_'',-1)+1)) like ''%BATCH%'' then
                                    SUBSTR (e2b_html_name, 1,INSTR (e2b_html_name, ''_'', -1) -1)||''.xml''
                                    END BATCH_FILE_NAME ';
      V_SQL_FROM      :=
            V_SQL_FROM
         || (CASE
   ...
   BEGIN
      V_SQL_QRY    :=
            'SELECT TRANSFORM_STATUS_ID, E.SAFETY_REPORT_ID,
					DECODE(E.REPORT_TYPE,''Upgrade'',E.REPORTING_DESTINATION,''Downgrade'',E.LOCAL_COMPANY_NAME) REPORTING_DESTINATION,
         ...
         || (CASE
                WHEN PI_REPORTING_DESTINATION IS NOT NULL
                   THEN    ' AND DECODE(E.REPORT_TYPE,''Upgrade'',UPPER(E.REPORTING_DESTINATION),''Downgrade'',UPPER(E.LOCAL_COMPANY_NAME)) =
                            '''
...
/*UPDATE E2B_TRANSFORM_STATUS E
       ...
       WHERE EXISTS (
                SELECT 1
                   FROM fda_argus_ack2 faa
                 WHERE faa.CORE_ID = PI_CORE_ID   
                 AND UPPER(faa.ORIGINAL_FILENAME) = UPPER(substr(E.E2B_FILE_NAME,instr(E.E2B_FILE_NAME,'\',-1,1)+1)) 
                 --AND FAA.MESSAGE_ID = E.MESSAGE_NUMBER                 
                   );
  ...
  FROM fda_argus_ack2 faa,
       relsysargus rl,
       MESSAGEEVENTSNAPSHOTS CYCM
WHERE     FAA.CORE_ID = PI_CORE_ID
       AND UPPER(faa.ORIGINAL_FILENAME) = UPPER(substr(E.E2B_FILE_NAME,instr(E.E2B_FILE_NAME,'\',-1,1)+1))
       AND UPPER (faa.ORIGINAL_FILENAME) = UPPER (CYCM.CONSUMPTIONFILENAME)
       AND CYCM.COREID = RL.REFTOCOREID
       AND UPPER (TRIM (rl.event_type)) = UPPER ('High')
       AND UPPER (TRIM (rl.event)) =
              UPPER ('Messaging.Message.ResponseDelivered')
       AND rl.event_date IS NOT NULL
       AND CYCM.RESPONSEPOSITIVE = 1 
	   AND CYCM.RESPONSECOREID IS NOT NULL); */
...
/******************************************************************************************************
       ...
       Parameters      :  PI_Msg_Type_ID            IN       NUMBER,
                          PI_SafetyReportID         IN       VARCHAR2,                         
                          PI_AGENCY_ID              IN       NUMBER,						 
						  PI_ATTACH_FILE_NAME       IN      VARCHAR2,
						  PI_ATTACH_FILE_CONTENT    IN      BLOB,
						  PI_ATTACHMENT_TYPE        IN      VARCHAR2,
						  PI_MESSAGE_NUMBER         IN      VARCHAR2
   ...
   *****************************************************************************************************************/      
   ...
   V_IMPORT_CASE_ATTACH         NUMBER;
   V_R3_XML_BLOB                BLOB            := NULL;
   V_IMPORT_CASE_ATTACH_CLASS_ID   NUMBER;
   V_IMP_ATTACHMENT_CLASS_ID    NUMBER;    
   V_IMPORT_CASE_ATTACH_CLID_LR    NUMBER;
   V_IMPORT_CASE_ATTACH_CLID_DHS    NUMBER;
  ...
  BEGIN
        ...
        SELECT VALUE INTO V_IMPORT_CASE_ATTACH FROM CFG_COMMON_PROFILE WHERE KEY = 'IMPORT_CASE_ATTACHMENT';
        
        SELECT VALUE INTO V_IMPORT_CASE_ATTACH_CLID_LR FROM CFG_COMMON_PROFILE WHERE KEY = 'IMPORT_CASE_ATTACH_CL_LR';
		
		SELECT VALUE INTO V_IMPORT_CASE_ATTACH_CLID_DHS FROM CFG_COMMON_PROFILE WHERE KEY = 'IMPORT_CASE_ATTACH_CL_DHS';
        ...
        IF PI_ATTACHMENT_TYPE = 'R3XML' AND V_IMPORT_XML_KEY = 1 THEN
        ...
        INSERT INTO E2B_IMPORT_QUEUE
                            ...
                            NULL,
                           PI_ATTACH_FILE_NAME,
                            PI_ATTACH_FILE_CONTENT,
                            V_IMP_ATTACHMENT_CLASS_ID,
							PI_MESSAGE_NUMBER,
							PI_ATTACH_FILE_NAME||' attached on '|| sysdate ||' for R3 import.'	                            
                            );
        ELSIF  PI_ATTACHMENT_TYPE = 'R2R3PDF' AND V_IMPORT_PDF_KEY = 1 THEN 
        ...
        INSERT INTO E2B_IMPORT_QUEUE
                            ...
                            V_IMP_ATTACHMENT_CLASS_ID,
							PI_MESSAGE_NUMBER,
							'R3 R2 view:'||PI_ATTACH_FILE_NAME||'. E2B imported document'									
                            );
                            
        ELSIF PI_ATTACHMENT_TYPE IN ('LR','DHS') AND V_IMPORT_CASE_ATTACH = 1 THEN -- CASE ATTACH
        ...
        INSERT INTO E2B_IMPORT_QUEUE
                            ...
                            PI_ATTACH_FILE_NAME,
                            PI_ATTACH_FILE_CONTENT,
                            decode(PI_ATTACHMENT_TYPE,'LR',V_IMPORT_CASE_ATTACH_CLID_LR,'DHS',V_IMPORT_CASE_ATTACH_CLID_DHS),
							PI_MESSAGE_NUMBER,
							decode(PI_ATTACHMENT_TYPE,'LR','Article '||PI_ATTACH_FILE_NAME||'. E2B imported document','DHS',PI_ATTACH_FILE_NAME)	
                            );
...
PROCEDURE P_E2B_IMPORT_FILE
    ...
    V_E2B_STATUS    NUMBER;
    V_SAFETYREPORTID VARCHAR2(3000);
    V_MESSAGE_NUMBER VARCHAR2(3000);
    V_CNT NUMBER;

    BEGIN

       FOR C2 IN ( select imp.*,substr(e.e2b_file_name,instr(e.e2b_file_name,'\',-1)+1) e2b_file_name
                    from e2b_import_queue imp,   E2B_TRANSFORM_STATUS e where e.safety_report_id = imp.safetyreportid and imp.message_number = e.message_number
                    and STATUS = 0 AND MSG_TYPE_ID = 1 )
        LOOP
         ...
         BEGIN
          ...
          --  V_SAFETYREPORTID := C2.SAFETYREPORTID;
           -- V_MESSAGE_NUMBER := C2.MESSAGE_NUMBER;
           
--            SELECT CASE_XREF,STATUS INTO V_CASE_ID,V_E2B_STATUS
--            FROM (SELECT CASE_XREF,S.STATUS
--            FROM &ESM_OWNER_USER..SAFETYREPORT S
--           WHERE SAFETYREPORTID = C2.SAFETYREPORTID
--           AND S.FILENAME = C2.e2b_file_name
--           ORDER BY DATE_IMPORTED DESC) WHERE ROWNUM = 1;
           
           
           
            SELECT CASE_XREF,S.STATUS
            INTO V_CASE_ID,V_E2B_STATUS
            FROM &ESM_OWNER_USER..SAFETYREPORT S, &ESM_OWNER_USER..MESSAGES M
           WHERE   SAFETYREPORTID = C2.SAFETYREPORTID
                 AND S.MSG_ID = M.MSG_ID
                 AND M.MESSAGEHEADER.MESSAGENUMB = C2.MESSAGE_NUMBER
                 AND UPPER(S.FILENAME) = UPPER(C2.E2B_FILE_NAME)
                 AND S.STATUS<>103;                          
         ...
         EXCEPTION
            ...
            THEN
               ...
               V_STATUS        := 1;
               V_MESSAGE       :=  'Case Rejected in Argus safety';
    ...
    ROLLBACK;
    --V_MESSAGE := SQLERRM||'S- ' || V_SAFETYREPORTID ||'M- '||V_MESSAGE_NUMBER;    
    RAISE;
    V_MESSAGE := SQLERRM||'S- ' || V_SAFETYREPORTID ||'M- '||V_MESSAGE_NUMBER;              
   ...
   /* V_CORE_ID   VARCHAR2 (500)
    ...
    FROM EDI_INFO EI, MESSAGES M
    WHERE EI.EDI_TRACKING_ID = M.EDI_TRACKING_ID
    AND M.EDI_TRACKING_ID = V_CORE_ID;*
     SELECT COUNT(1) INTO V_EDI_CNT
    FROM fda_argus_ack2 faa
    WHERE faa.CORE_ID = P_CORE_ID;*/
 SELECT COUNT(1) INTO V_EDI_CNT
    FROM EDI_INFO EI
...
PROCEDURE P_SAVE_E2B_R3VIEW_STATUS (
    PI_MESSAGE_NUMBER IN PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_SAFETY_REPORT_ID IN PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_R3_FILE_NAME IN PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_LOGIN_USER_NAME IN  PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_ACCEPT_REJECT_FLAG IN PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_ACCEPT_REJECT_NOTES IN PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_R3_BATCH_NAME IN PKG_COMMON_UTILITIES.T_STRING_TAB,
    PI_MSGN_SAFETYID_FNAME IN PKG_COMMON_UTILITIES.T_STRING_TAB
)
...
BEGIN
    ...
    FOR I IN PI_MESSAGE_NUMBER.FIRST .. PI_MESSAGE_NUMBER.LAST
      LOOP
          BEGIN
            
          INSERT INTO E2B_R3VIEW_STATUS
          (MESSAGE_NUMBER, SAFETY_REPORT_ID, R3_FILE_NAME, LOGIN_USER_NAME, ACCEPT_REJECT_FLAG, ACCEPT_REJECT_NOTES, R3_BATCH_NAME, DELETED, DELETED_DATE,
          MSGN_SAFETYID_FNAME,INSERT_DATE)
          VALUES
          (
          PI_MESSAGE_NUMBER(I),
          PI_SAFETY_REPORT_ID(I),
          PI_R3_FILE_NAME(I),
          PI_LOGIN_USER_NAME(I),
          PI_ACCEPT_REJECT_FLAG(I),
          PI_ACCEPT_REJECT_NOTES(I),
          PI_R3_BATCH_NAME(I),
          0,
          NULL,
          PI_MSGN_SAFETYID_FNAME(I),
          SYSDATE
          );
        ...
        EXCEPTION WHEN NO_DATA_FOUND THEN
          ...
          NULL;--  PKG_LOGGER.P_LOG_TRACE($$PLSQL_UNIT,'P_DELETE_R2R3_MAPPING','PI_PROFILE_ID=> '||PI_PROFILE_ID||', PI_MAPPING_ID_LIST=>'||PI_MAPPING_ID_LIST(I)||',PI_USER_ID=>'||PI_USER_ID,1,3,'Trace');

        END;
         
      END LOOP;
    
END P_SAVE_E2B_R3VIEW_STATUS;

PROCEDURE P_GET_E2B_R3VIEW_STATUS
(PI_MSGN_SAFETYID_FNAME IN PKG_COMMON_UTILITIES.T_STRING_TAB,
PO_R3_DETAILS  OUT   SYS_REFCURSOR
)
AS
BEGIN

 PKG_COMMON_UTILITIES.GV_SQL_TYP_TAB := PKG_COMMON_UTILITIES.F_CONV_PLSQL_TO_SQL_TAB (PI_MSGN_SAFETYID_FNAME) ;
    
 OPEN PO_R3_DETAILS FOR SELECT SOURCE_MSG_SAFETYID_FNAME MSG_SAFETYID_FNAME, ACCEPT_REJECT_FLAG, 
 DECODE(ACCEPT_REJECT_FLAG,1,'This file is accepted by '||LOGIN_USER_NAME || ' On '||ERS.INSERT_DATE ,2,'This file is rejected by '||LOGIN_USER_NAME||' On '||ERS.INSERT_DATE) ACCEPT_REJECT_NOTES,
 INSERT_DATE PROCESSED_DATE
  FROM E2B_R3VIEW_STATUS ERS,  
            (SELECT COLUMN_VALUE SOURCE_MSG_SAFETYID_FNAME
              FROM   TABLE( PKG_COMMON_UTILITIES.GV_SQL_TYP_TAB)
             ) T2
   WHERE ERS.MSGN_SAFETYID_FNAME = T2.SOURCE_MSG_SAFETYID_FNAME
   AND ERS.DELETED =0;          
